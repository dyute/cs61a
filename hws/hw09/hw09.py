...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'cs\s?\d+[abc]*', post, flags=re.IGNORECASE))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'\b(?:(?:[0-1]?\d)|(?:2[0-3])):[0-5]\d(?:AM|PM)?\b', text)
...
def most_common_code(text):
    ...
    '123'
    """
    code = area_codes(text)
    return max(code, key=lambda x: code.count(x))
