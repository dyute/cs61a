...
def missing_digits(n):
    ...
    diff = n % 10 - prev % 10
    if diff > 1:
        return diff - 1 + missing_digits(prev)
    else:
        return missing_digits(prev)
...
def count_coins(change):
    ...
    def count(change, max_coin=25):
        ...
        if change == 0:
            return 1
        return count(change - max_coin, max_coin) + count(change, descending_coin(max_coin))
    return count(change)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda f: f(f))(lambda f: lambda x: 1 if x == 0 else mul(f(f)(sub(x, 1)), x))
