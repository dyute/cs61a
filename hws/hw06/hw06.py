...
class VendingMachine:
    ...
    def vend(self):
        ...
        self.stock, self.balance = self.stock - 1, 0
        if change == 0:
...
def deep_map_mut(fn, link):
    ...
    node = link
    while node:
        if isinstance(node.first, Link):
            deep_map_mut(fn, node.first)
        else:
            node.first = fn(node.first)
        node = node.rest
...
class VirFib():
    ...
    def next(self):
        next_value = 1 if self.value == 0 else self.value + self.prev
        next_ins = VirFib(next_value)
        next_ins.prev = self.value
        return next_ins
...
def is_bst(t):
    ...
    def bst_extre(t, f):
        if t.is_leaf():
            return t.label
        return f(t.label, f([bst_extre(b, f) for b in t.branches]))

    def bst_min(t):
        return bst_extre(t, min)
    
    def bst_max(t):
        return bst_extre(t, max)
