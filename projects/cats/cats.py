...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    ps = [p for p in paragraphs if select(p)]
    return ps[k] if k < len(ps) else ''
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    if not typed_words:
        return 0.0 if reference_words else 100.0
    correct = sum([1 for t, r in zip(typed_words, reference_words) if t == r])
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    word = min(valid_words, key=lambda w: diff_function(typed_word, w, limit))
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0 or not start or not goal:  # Fill in the condition
        # BEGIN
        return 0 if limit < 0 else len(start) or len(goal)
        # END
...
def time_per_word(words, times_per_player):
    ...
    for t in times_per_player:
        times.append([t[i] - t[i - 1] for i in range(1, len(t))])
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            ...
            if t < min_t:
                min_t, player = t, j 
        fastest[player].append(word_at(match, i))
    return fastest 
    # END PROBLEM 10
