...
def picky_piggy(score):
    ...
    """
    # BEGIN PROBLEM 2
    digits = 142857  # decimal part of 1/7
    if score == 0:
        return 7
    n = (score - 1) % 6 + 1
    while n < 6:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        record_gain = running_high
        if gain > record_gain:
            print(f"{gain} point(s)! That's a record gain for Player {who}!")
            record_gain = gain
        return announce_highest(who, score, record_gain)
    return say
    # END PROBLEM 7
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    points = picky_piggy(opponent_score)
    if points >= cutoff:
        return 0
    else:
        return num_rolls
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    total_score = score + picky_piggy(opponent_score)
    if total_score == opponent_score:
        return 0
    dice = picky_piggy_strategy(score, opponent_score, cutoff, num_rolls)
    if dice == 0:
        return 0
    return num_rolls
    # END PROBLEM 11
