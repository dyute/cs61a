...
def do_and_form(expressions, env):
    ...
    while expressions is not nil: 
        test, expressions = scheme_eval(expressions.first, env), expressions.rest
        if is_scheme_false(test):
            return test
    return test
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        item, bindings = bindings.first, bindings.rest
        validate_form(item, 2, 2)
        names = Pair(item.first, names)
        values = Pair(scheme_eval(item.rest.first, env), values)
