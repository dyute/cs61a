...
def prune_min(t):
    ...
    if t.is_leaf():
        return
    t.branches = [min(t.branches, key=lambda t: t.label)]
...
class Player:
    ...
    def debate(self, other):
        ...
        p = max(0.1, p1 / (p1 + p2))
        self.popularity = p1 + 50 if p > random() else max(0, p1 - 50)
...
class Game:
    ...
    def play(self):
        while not self.game_over:
            player, other = (self.p2, self.p1) if self.turn % 2 else (self.p1, self.p2)
...
def add_trees(t1, t2):
    ...
    if not t1 or not t2:
        return t1 or t2
    label, branches = t1.label + t2.label, []
    for b1, b2 in zip(t1.branches, t2.branches):
        branches.append(add_trees(b1, b2))
    n = len(branches)
    branches.extend(t1.branches[n:] or t2.branches[n:])
